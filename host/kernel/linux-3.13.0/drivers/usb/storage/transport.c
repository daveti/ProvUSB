...
/* daveti: TPM attestation routines */
static int fsg_trusted_dev_trans_debug = 1;

/* Get the nonce and PCR mask from the trusted device */
int usb_stor_Ctrl_get_nonce(struct us_data *us, struct us_trusted_dev_ctrl *block)
{
        int result;
        ...
        /* Prepare the iobuf */
        memset(us->iobuf, 0x0, US_TRUSTED_DEV_TPM_ATT_REQ_LEN);
        ...
        result = usb_stor_control_msg(us, us->recv_ctrl_pipe,
                                ...
                                US_TRUSTED_DEV_CTRL_TRANS_TIMEOUT*HZ);

        usb_stor_dbg(us, "daveti2: ADSC command (get_nonce) result is [%d]\n", result);

        /* Save the noce and PCR mask into trusted dev */
        memcpy(block->iobuf, us->iobuf, US_TRUSTED_DEV_TPM_ATT_REQ_LEN);

        return result;
}

/* Send the TPM quote results to the trusted device */
/* NOTE: do NOT use this API to send the ADSC request
 * Instead, use usb_stor_Ctrl_send_adsc()
 * Aug 7, 2014
 * daveti
 */
int usb_stor_Ctrl_send_quote(struct us_data *us, struct us_trusted_dev_ctrl *block)
{
        int result;
        int data_len;
        void *data;
        ...
        if (block->tpm_quote_store.aik_pub_key) {
                /* Use AIK */
                if (block->tpm_quote_store.sent_len == 0) {
                        data = (void *)(block->tpm_quote_store.aik);
                        data_len = US_TRUSTED_DEV_ADSC_MAX_PKT_SIZE;
                }
                else {
                        data = (void *)(block->tpm_quote_store.aik +
                                        US_TRUSTED_DEV_ADSC_MAX_PKT_SIZE);
                        data_len = US_TRUSTED_DEV_TPM_ATT_LAST_AIK_LEN;
                }
        }
        else {
                /* Use quote */
                if (block->tpm_quote_store.sent_len == 0) {
                        data = (void *)(block->tpm_quote_store.quote);
                        data_len = US_TRUSTED_DEV_ADSC_MAX_PKT_SIZE;
                }
                else {
                        data = (void *)(block->tpm_quote_store.quote +
                                        US_TRUSTED_DEV_ADSC_MAX_PKT_SIZE);
                        data_len = US_TRUSTED_DEV_TPM_ATT_LAST_QUOTE_LEN;
                }
        }

        /* Debug */
        if (fsg_trusted_dev_trans_debug) {
                printk(KERN_INFO "daveti2: dump the data with len [%d]\n", data_len);
        ...
        /* Defensive checking */
        if (result != data_len)
                printk(KERN_ERR "daveti: sent [%d] but ack'd [%d]\n", data_len, result);

        /* Update the length */
        block->tpm_quote_store.sent_len += data_len;

        /* Debug */
        if (fsg_trusted_dev_trans_debug)
                printk(KERN_INFO "daveti2: sent_len [%d]\n",
                        block->tpm_quote_store.sent_len);

        usb_stor_dbg(us, "daveti2: ADSC command (send_quote) result is [%d]\n", result);
        return result;
}

/* Send the TPM quote results to the trusted device */
int usb_stor_Ctrl_send_adsc_debug(struct us_data *us, u8 *data, int len, int use_iobuf)
{
        int result;

        printk(KERN_INFO "daveti2: into [%s]\n"
                "data_len [%d], use_iobuf [%d]\n", __FUNCTION__, len, use_iobuf);

        if (use_iobuf) {
                printk(KERN_INFO "daveti: using iobuf\n");
                /* Caller should guarantee that the data is not exceeding iobuf */
                memcpy(us->iobuf, data, len);
        ...
        usb_stor_dbg(us, "daveti2: ADSC command (debug) result is [%d]\n", result);
        return result;
}

/* General routine to send the ADSC cmd (OUT transfer) */
int usb_stor_Ctrl_send_adsc(struct us_data *us, u8 *data, int len)
{
        int result;

        printk(KERN_INFO "daveti2: into [%s], data_len [%d]\n", __FUNCTION__, len);
        ...
        usb_stor_dbg(us, "daveti2: ADSC command (OUT) result is [%d]\n", result);
        return result;
}
