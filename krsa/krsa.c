 ...
 */
static int krsa_verify(struct krsa_ctx *ctx)
{
	size_t tsize;
	int ret;
...
static int __init krsa_init(void)
{
	int ret;
	...
	/* Init */
	ret = krsa_ctx_init(&rsa_ctx,
			(u8 *)tpm_quote_sig, KRSA_TPM_QUOTE_SIG_LEN,
			(u8 *)tpm_quote_valid, KRSA_TPM_QUOTE_VALID_LEN,
			(u8 *)tpm_aik_pub_key_n, KRSA_TPM_AIK_PUB_KEY_N_LEN,
			(u8 *)tpm_aik_pub_key_e, KRSA_TPM_AIK_PUB_KEY_E_LEN);
	if (ret) {
	...
	/* Digest */
	ret = krsa_make_digest(&rsa_ctx, HASH_ALGO_SHA1);
	if (ret) {
	...
	if (krsa_debug)
		print_hex_dump(KERN_INFO, "digest_", DUMP_PREFIX_ADDRESS, 16, 1,
				rsa_ctx.digest, rsa_ctx.digest_size, 1);
	...
	/* Verify */
	ret = krsa_verify(&rsa_ctx);
	if (ret)
		pr_err("verification failed\n");
	else
		pr_info("verification succeeded\n");
...
static void __exit krsa_exit(void)
{
	pr_info("exiting krsa module\n");
	krsa_ctx_destory(&rsa_ctx);
}

module_init(krsa_init);
module_exit(krsa_exit);
